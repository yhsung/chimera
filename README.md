...
| RISCV-Linux | 4 / 2 GB | QEMU `virt` rv64, OpenSBI | Debian Linux 12 (bookworm) | 172.16.100.11 | `debian-riscv64.local` | Runs `syslog-riscv-linux` (sysinfo → FreeRTOS), waits for ACK |
| MIPS-Linux | 1 / 256 MB | QEMU `malta` mipsel | Debian Linux 12 (bookworm) | 172.16.100.12 | `debian-mipsel.local` | Runs `syslog-mips-linux` (sysinfo → FreeRTOS), waits for ACK |
